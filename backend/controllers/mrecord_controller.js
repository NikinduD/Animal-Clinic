...
const createMedicalRecord = async (req, res) => {
  try {
    const { petName, date, diagnosis, treatment,weight} = req.body;
    const newMedicalRecord = new MedicalRecord({ petName, date, diagnosis, treatment,weight });
...
const updateMedicalRecord = async (req, res) => {
  try {
    ...
    const updatedMedicalRecord = await MedicalRecord.findByIdAndUpdate(
      medicalRecordId,
      { petName, date, diagnosis, treatment,weight },
      { new: true }
    );
